...
// API handlers
async function handleSimplePrice(ids, vs_currencies, include_market_cap = false, include_24hr_vol = false, include_24hr_change = false) {
    try {
...
async function handleCoinData(id, vs_currencies = ["usd"]) {
    try {
    ...
    try {
        switch (request.params.name) {
            case "coingecko_price": {
                const { ids, vs_currencies, include_market_cap, include_24hr_vol, include_24hr_change } = request.params.arguments;
                return await handleSimplePrice(ids, vs_currencies, include_market_cap, include_24hr_vol, include_24hr_change);
            }
            case "coingecko_list": {
                const { include_platform } = request.params.arguments;
                return await handleCoinList(include_platform);
            }
            case "coingecko_coin_data": {
                const { id, vs_currencies } = request.params.arguments;
